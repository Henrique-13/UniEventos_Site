...
/* ════════════════════════════════════════
   UNIEVENTOS — checkout.js
   Fluxo completo:
   1. reCAPTCHA v3 — verificação invisível
───────────────────────────────────── */

const RECAPTCHA_SITE_KEY = '6Lebie8sAAAAAPHUsVm-Hn79L8Tyq4FIcnml51w8';

function openCaptcha(pkgKey) {
  currentPackageKey = pkgKey;

  const html = `
    <div class="ue-captcha-header" style="text-align:center;padding:48px 36px;">
      <div class="ue-captcha-logo">UNI<span>EVENTOS</span></div>
      <div style="margin:24px auto 16px;width:56px;height:56px;border-radius:50%;
                  background:rgba(245,200,66,0.1);border:2px solid rgba(245,200,66,0.3);
                  display:flex;align-items:center;justify-content:center;font-size:26px;">
        🛡️
      </div>
      <div class="ue-captcha-title">Verificando segurança...</div>
      <p class="ue-captcha-sub" style="margin-top:8px;">
        Aguarde um instante, estamos confirmando que você é humano.
      </p>
      <div id="ue-recaptcha-error" style="display:none;margin-top:16px;
           background:#fff0f0;border:1px solid #ffcccc;color:#c0392b;
           font-size:12px;font-weight:600;padding:10px 16px;border-radius:8px;">
        ❌ Verificação falhou. Por favor, tente novamente.
      </div>
      <button class="ue-btn ue-btn-ghost" onclick="closeModal()" style="margin-top:20px;">
        Cancelar
      </button>
    </div>
  `;
  ...
  grecaptcha.ready(async () => {
    try {
      const token = await grecaptcha.execute(RECAPTCHA_SITE_KEY, { action: 'reserva' });
      if (token) {
        openCheckout(pkgKey);
      } else {
      ...
      console.warn('[UniEventos] reCAPTCHA falhou:', err);
      const errEl = document.getElementById('ue-recaptcha-error');
